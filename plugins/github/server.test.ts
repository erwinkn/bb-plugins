...
describe("GitHub RPC contract", () => {
  ...
  it("rejects CLI arguments that would otherwise broaden a repository query", () => {
    ...
    expect(validateGithubCliArgs(["link"])).toContain("needs a pull request");
    expect(validateGithubCliArgs(["links", "x"])).toContain("does not accept arguments");
    expect(validateGithubCliArgs(["link", "#12"])).toBeNull();
  });

  it("splits --thread out of argv before validation", () => {
    expect(splitThreadFlag(["link", "#12", "--thread", "thr_1"])).toEqual({ argv: ["link", "#12"], threadId: "thr_1" });
    expect(splitThreadFlag(["--thread=thr_2", "links"])).toEqual({ argv: ["links"], threadId: "thr_2" });
    expect(splitThreadFlag(["links", "--thread"]).error).toContain("needs a thread id");
  });
