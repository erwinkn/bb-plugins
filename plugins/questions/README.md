...
1. The agent calls `questions_ask`. BB creates a pending interaction and marks
   the thread as needing input. Each interaction lasts one hour. The plugin
   renews it on timeout while the tool remains active, using the same round.
2. A panel round shows a compact card with **Open** in the interaction area;
   an inline round shows its questions there. A new panel round also opens
   the panel once through the thread header control. Existing message
   directives remain valid for reopening saved rounds.
...
4. **Submit** submits the selected round. Every required question must have
   an answer. Questions with `optional: true` may be left blank and are stored
   as skipped. Both the UI and server reject partial round submissions.
5. The waiting tool returns the frozen answers, without also sending a user
   message. The agent can read stored answers with `questions_read`, and can
...
- `questions_ask` `{ mode?, intro?, questions: [{ title, optional?, help?, options?, select?, cites?, attachments?, references?, confidence? }] }`.
  ...
  Questions are required by default. Only one native interaction can wait in
  a thread at a time. Tool text is bounded to 512 KiB; larger rounds return
  a pointer to `questions_read` and its pagination cursors.
- `questions_image` `{ question, path }` returns one submitted image as image
  content. `question` accepts a Q label or ID. It rejects draft-only images
  and paths owned by another question or thread.
...
The CLI always waits through the same native interaction as the tool.
The thread defaults to the thread the command
...
- **Submission** freezes the whole round into an outbox row with a client-generated
  id before resolving the native interaction or sending a late-answer message.
  ...
  delivery (`sent` for a resolved interaction or sent message, or `queued`
  for a late message when the agent is busy).
- **Uncertain delivery**: when the server does not confirm delivery, the
  record becomes `uncertain` and drafts remain. The panel asks the user to
  check the submission id in the thread before submitting the complete round
  again. There are no frozen-answer Retry controls, partial retries, or retry
  chains. A repeat request after a transport failure reuses its request id
  to read the stored result instead of sending twice. A new submission after
  an uncertain result can duplicate answers; it is never sent automatically.
...
- Attachments (only on questions asked with `attachments: true`) are uploaded
  to the project's prompt attachments through BB's SDK. Native tool results
  contain their paths; `questions_image` lets the agent view submitted images
  one at a time. Late-answer messages include `localImage` or `localFile`
  parts. Limits: 8 MB per file, 10 files per
...
- Hourly timeouts renew the native interaction automatically, without another
  agent turn or a new round. The indicator can briefly clear between requests.
  Cancellation, stopping the thread, plugin disposal, or server restart ends
  the wait and does not renew it. If renewal fails, the tool reports the error;
  saved drafts remain and later complete-round submissions use message delivery.
- Message delivery is only a fallback when no live interaction owns the round.
  A native response error never triggers an automatic message send. Historical
  submissions and the unused `retry_of` database column remain to preserve data,
  but `retryOf`, `canRetry`, and the create-without-waiting CLI path are removed
  from the active API. Old message directives are read-only reopening controls.
- Old rounds without an optional flag treat every question as required.
  Their submitted answers and drafts remain unchanged.
