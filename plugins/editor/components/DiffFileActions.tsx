...
export function DiffFileActions({ children, entry, target, threadId, onChanged }: {
  ...
  const prepare = async () => {
    ...
    try {
      ...
      if (!mounted.current) return finish();
      ...
      const run = async () => {
        ...
        try {
          const action = async () => {
            const result = await rpc.call("diffRevert", {
              threadId,
              target,
              path: entry.path,
              expectedSha256: data.sha256,
              expectedBaselineSha256: data.baselineSha256,
      ...
      if (deleting || snapshot?.dirty || (snapshot && snapshot.draft.kind !== "none")) {
        setBusy(false);
        setConfirmation({ deleting, run });
      } else {
        await run();
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not read the comparison");
      finish();
    }
  };
  ...
  return (
    ...
    >
      ...
      {confirmation ? (
        <div
          role="alertdialog"
          aria-label={confirmation.deleting ? `Delete ${entry.path}?` : `Revert ${entry.path}?`}
        ...
        >
          <p>
            {confirmation.deleting
              ? `Delete ${entry.path}? This also removes any unsaved edits.`
              : `Revert ${entry.path} and discard its unsaved edits?`}
          </p>
          <div className="mt-2 flex gap-2">
            <button type="button" autoFocus disabled={busy} onClick={finish} className="rounded px-2 py-1 hover:bg-state-hover disabled:opacity-50">
              Cancel
            </button>
            <button
              type="button"
              ...
              className="rounded bg-destructive px-2 py-1 text-white disabled:opacity-50"
            >
              {busy ? "Working…" : confirmation.deleting ? "Delete file" : "Revert file"}
            </button>
          </div>
        </div>
      ) : null}
