...
test("realtime tools expose bounded quick actions without destructive or arbitrary tools", () => {
  assert.deepEqual(liveToolSchemas().map(tool => tool.name), ["list_machines", "list_models", "find_targets", "read_threads", "message_thread", "spawn_worker", "create_thread", "prepare_draft", "control_ui", "stop_thread", "queued_messages", "rename_thread", "update_thread", "subscriptions", "prepare_archive", "archive_threads", "answer_interaction", "remain_silent", "end_call"]);
});
