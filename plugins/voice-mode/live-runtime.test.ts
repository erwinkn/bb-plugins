...
test("live tools expose all nineteen strict argument schemas", () => {
  const schemas = liveToolSchemas(); assert.equal(schemas.length, 19); assert.equal(new Set(schemas.map(s => s.name)).size, 19);
