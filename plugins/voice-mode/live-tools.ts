...
import { permissionModeSchema } from "./permission-mode.ts";
...
const permission = {
  permission_mode: permissionModeSchema.optional().describe("Override the Voice Mode/profile permission default only when the user explicitly requested it. auto means Approve for me; full bypasses BB's sandbox and approval protections."),
  permission_confirmed: z.boolean().optional().describe("Required and true only when permission_mode is full and the user explicitly authorized full access for this launch."),
};
const launchArgs = <T extends z.ZodRawShape>(shape: T) => z.object({ ...shape, ...execution, ...workspace, ...permission }).strict().superRefine((args, ctx) => {
  const permissionArgs = args as { permission_mode?: string; permission_confirmed?: boolean };
  if (permissionArgs.permission_mode === "full" && permissionArgs.permission_confirmed !== true) {
    ctx.addIssue({ code: "custom", path: ["permission_confirmed"], message: "Full access needs explicit confirmation for this launch." });
  }
});
export const liveToolArgs = {
  list_machines: z.object({}).strict(),
  ...
  spawn_worker: launchArgs({ profile: z.string().min(1).max(64).optional().describe("A configured profile name. Omit for the default profile."), title: z.string().min(1).max(200), task: z.string().min(1).max(24000).describe("Task, context, constraints, and expected result."), project_id: id.optional().describe("Only when the task needs that repository's files. Omit to run outside any project."), host_id: id.optional().describe("Machine ID from list_machines or call-start context. Omit for BB's primary machine when running outside a project. For computer use, select the user's intended device explicitly.") }),
  ...
  control_ui: z.object({ action: z.enum(["open_thread", "open_project", "preview_file", "show_voice", "switch_space"]), thread_id: id.optional(), project_id: id.optional(), path: z.string().min(1).max(4096).optional(), source: z.enum(["workspace", "thread-storage"]).optional(),
    space: z.string().min(1).max(120).optional().describe("With switch_space: the space name as spoken, \"all projects\", or \"library\".") }).strict(),
...
const descriptions: Record<LiveTool, string> = {
  list_machines: "List enrolled machines with their names, IDs, connection status, and BB's default worker machine. Use before choosing where a worker runs, especially for computer use on the user's device. Refresh if a machine disconnects. A browser's platform does not identify its BB machine.",
  ...
  spawn_worker: "Start a hidden background worker. It runs outside any project on BB's primary machine by default; optional host_id selects another connected machine from list_machines. It can inspect every BB project and thread; give project_id only when the task needs that repository's files. For computer use on the user's device, choose that machine and provider codex. A spoken provider, model, or reasoning level overrides the profile; workspace picks a new worktree, the main folder, or another thread's worktree. It uses the profile's permission mode, inheriting the destination project/BB default when configured; an explicit permission_mode overrides it, and full needs permission_confirmed after explicit user authorization. Supply context, constraints, and expected result in task. Returns launch status, not completion; the result, failure, or question is reported to you automatically in this call. Hidden only means it is not listed in the sidebar.",
  ...
  control_ui: "Navigate on the call owner device. Open a thread or project, preview a file, show Voice, or switch_space to change the Threads sidebar to a saved space by its spoken name (all projects, or the library of saved threads). The result names the scope that was applied, or lists the saved spaces when none matched. Background updates cannot navigate.",
...
export function liveToolSchemas(options: LiveToolOptions = {}) {
  ...
  return (Object.keys(liveToolArgs) as LiveTool[]).map(name => {
    ...
    if (name === "spawn_worker" && names.length > 0) {
      ...
      schema = liveToolArgs.spawn_worker.safeExtend({ profile: z.enum(names as [string, ...string[]]).optional().describe(`A configured profile name. Omit for ${fallback}.`) }).strict();
    }
