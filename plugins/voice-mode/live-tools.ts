...
const descriptions: Record<LiveTool, string> = {
  ...
  spawn_worker: "Start a hidden background worker. It runs outside any project on the primary machine by default and can inspect every BB project and thread; give project_id only when the task needs that repository's files. Supply context, constraints, and expected result in task. Returns launch status, not completion.",
