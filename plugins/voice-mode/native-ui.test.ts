...
test("switch_space resolves a spoken name from the cached catalog, applies it, and tells the activity store", async () => {
  ...
  try {
    ...
    values.set("bb-plugin-sidebar:spaces-cache", JSON.stringify({ revision: 1, spaces: [{ id: "s1", name: "Mobile", projectIds: ["a", "b"] }, { id: "s2", name: "BB plugins", projectIds: ["c"] }] }));
    values.set("bb-plugin-sidebar:v1", JSON.stringify({ groupBy: "status", spaceId: null }));
    ...
    assert.equal(JSON.parse(values.get("bb-plugin-sidebar:v1")!).spaceId, "s1");
    assert.deepEqual(events, ["bb-plugin-sidebar:state"]);
    ...
    assert.equal(JSON.parse(values.get("bb-plugin-sidebar:v1")!).spaceId, "s1", "a miss changes nothing");
    assert.match((await ui.execute({ kind: "switch_space", space: "all projects" }, current)).detail, /all projects/);
    assert.equal(JSON.parse(values.get("bb-plugin-sidebar:v1")!).spaceId, null);
