...
App registration runs in each client. Only mobile clients register the Voice Mode
...
- Voice page: its fixed drawer view through `experimental_useAppPanel`.
...
1. On mobile, start from Voice Mode; open/swap threads, dismiss/reopen the drawer,
...
5. Change the mobile preference in settings and by voice. On desktop, verify
   `focus_thread` still navigates from both Voice Mode and a composer regardless.
