...
test("usage states handle missing CLI and probe failures without exposing errors", async () => {
  ...
  const ok = await getDevinUsage("custom-devin", { executable: async c => c, probe: async c => { assert.equal(c, "custom-devin");return sample; } });
...
test("bridge delegates session traffic and lifecycle; usage routing validates input", async () => {
  ...
  }), async command => { assert.equal(command, "/custom/devin");return { supported: true, usage: { status: "unauthenticated" } }; }, line => outputs.push(JSON.parse(line)));
  ...
  await new Promise(resolve => setImmediate(resolve));bridge.onClose?.();
