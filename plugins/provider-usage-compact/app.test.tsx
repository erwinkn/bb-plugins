...
describe("provider usage footer disclosure", () => {
  it("aggregates every machine and keeps machine and provider selection local to the card", async () => {
    const extraProviders = ["Claude Code", "Cursor", "Devin", "Extra one", "Extra two"].map((name) => ({
      ...
      signInHint: "Sign in.", expiredHint: "Sign in again.",
      usage: { status: "ok", accountEmail: null, planLabel: "Pro", windows: [
        { label: "Weekly limit", usedPercent: 99, resetsAt: null, cost: null },
      ] },
    }));
    ...
    const machinePicker = await slot.findByRole("button", {
      name: "Usage machine: A machine with a name too long for the header",
    });
