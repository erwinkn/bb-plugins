...
`lib/host-icon-names.ts` is generated by the repository's `scripts/host-icon-names.mjs` from the pinned bb tag and types every icon name.
Regenerate it on each bb upgrade (`node scripts/host-icon-names.mjs --tag <bb-tag> --plugin plans`).
