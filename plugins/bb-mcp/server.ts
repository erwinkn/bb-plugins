...
export default function plugin(bb: BbPluginApi) {
  ...
  const handler = createMcpHandler(() => {
    const server = new McpServer({ name: "bb-mcp", version: "0.2.1" }, {
      capabilities: { tools: {} },
    ...
    tool("bb_list_projects", "List projects in this connection's scope. Empty project scope includes all ordinary and personal projects.", z.object({}).strict(), false, adapter.listProjects);
    tool("bb_get_capabilities", "Inspect implemented features, execution-control limits, product-only controls, and the parity roadmap. Planned features are not callable yet.", z.object({}).strict(), false, async () => ({ version: "0.2.1", features: parity, permissionCeiling: (await settings.get()).permissionMode, updateExecutionFields: ["model", "reasoningLevel"], createAndSendExecutionFields: Object.keys(execution) }));
    tool("bb_list_runtimes", "List hosts and installed harnesses in this connection's scope. Empty host/provider scopes include all available entries. Pass providerId to page its models; environmentId resolves workspace-specific catalogs.", z.object({ projectId: id, hostId: id.optional(), environmentId: id.optional(), providerId: id.optional(), ...page }).strict(), false, adapter.listRuntimes);
    tool("bb_list_threads", "List project threads, including hidden threads by default. Optionally filter parent/source, roots vs children, archived state, or visibility. Title and host filters apply to scanned pages; follow nextOffset even if no rows match.", z.object({ projectId: id, query: z.string().max(200).optional(), parentThreadId: id.optional(), sourceThreadId: id.optional(), hasParent: z.boolean().optional(), archived: z.boolean().optional(), includeHidden: z.boolean().default(true), ...page }).strict(), false, adapter.listThreads);
