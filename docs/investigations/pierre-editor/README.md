...
| Change detection | There is no active file watch. An agent edit is detected at save through the hash check or by manual reload. A renderer replacement alone does not fix this. |
...
4. Add a host watch module, its manifest entry and client invalidation. Test local and remote hosts, rescan, disconnect, atomic file replacement and agent edits during typing. Add separate staged/unstaged views only after a host Git adapter or upstream API exists. Defer stage/revert controls until stale index/disk and partial-hunk tests pass. Commit snapshots stay read-only.
